...
fn parse_span_color(tag: &str) -> Option<egui::Color32> {
    let idx = tag.find("color:")? + "color:".len();
    let hex = tag[idx..].trim_start().strip_prefix('#')?;
    ...
    fn event(
        ...
        match event {
            ...
            pulldown_cmark::Event::InlineHtml(html) => {
                // Trellis patch: honor `<span style="color:#rrggbb">…</span>` so
                // the color button shows live (upstream drops all inline HTML).
                let tag = html.trim();
                if tag.starts_with("</span") {
                    self.text_style.color = None;
                } else if tag.starts_with("<span") {
                    if let Some(color) = parse_span_color(tag) {
                        self.text_style.color = Some(color);
                    }
                }
            }
