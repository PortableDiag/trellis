...
- **Arrange** — with two or more cards selected, the *Arrange* menu aligns them
  (left/centre/right/top/middle/bottom), distributes them at even gaps, or re-lays
  them as a row, column or grid. Unlike Autosort it touches only the selection.
- **Callouts** — `> [!note]`, `> [!tip]`, `> [!warning]`, `> [!info]`, `> [!bug]`
  and the rest of the Obsidian set render as titled, coloured blocks, including a
  custom title written on the same line.
...
- **Properties** — inline `key:: value` fields in a card (e.g. `due:: 2026-08-15`,
  `status:: open`) are parsed as metadata you can query across the tree. Writing
  *about* one doesn't create one: inside backticks or a fenced code block it is
  quoted text, so notes that document the syntax stay off your agenda.
...
- **Agent API** — a key-gated HTTP API (localhost by default; opt-in **LAN
  ...
  **reorder and reparent nodes** (`/nodes/{id}/move`), **reorder cards** within a
  ...
  every 4xx/5xx the API answered, with who asked, **the address they asked
...
elsewhere on the LAN can read the reference without this machine's filesystem, and
what it reads describes the version actually serving it rather than whatever happens
to be checked out.
...
**A card can be a conversation.** Right-click a card → **Make a channel…** (or the
card reader's **Channel…** on Android). Give one a `channel` and its body becomes a
