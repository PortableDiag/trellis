                ...
                egui::ScrollArea::vertical()
                    ...
                    ui.collapsing("Endpoints", |ui| {
                        ...
                        "POST   /api/cube       {nodes:[<basket ids>]}  (open a compressed-workspace cube over those baskets, in order, first deepest — a temporary view of embeds, nothing stored; DELETE /api/cube leaves it)",
                            ...
                            "GET    /api/cards/{cid}                   (find a card from its id alone → {node, node_path, card} — the body is card.body, not .body; to ADD to a card use …/append and never read it)",
                        ...
                        "         a bare [[Title]] resolves to the linking card's own project first, then the lowest node id (stable across runs)",
                            ...
                            "GET    /api/cards/{cid}/images   (what this card carries: names + sizes, never bytes; inline body images listed separately)  ·  POST /api/cards/{cid}/images {data_base64}  ·  GET /api/cards/{cid}/images/{idx}  ·  DELETE /api/cards/{cid}/images/{idx}",
                            ...
                            "GET    /api/nodes/{id}/cards/{cid}/images  (names + sizes)  ·  GET /api/nodes/{id}/cards/{cid}/images/{idx}  ·  DELETE /api/nodes/{id}/cards/{cid}/images/{idx}",
